...
class InboxApi {
    func lastMessages(uid: String, onSuccess: @escaping(InboxCompletion)) {
        ...
        // 8: number of messages loaded when app is first launched
    ...
    func loadMore(start timestamp: Double?, controller: MessagesTableViewController, from: String, onSuccess: @escaping(InboxCompletion)) {
        ...
        // to load more user with the date less than timestamp
        let ref = Database.database().reference().child(REF_INBOX).child(from).queryOrdered(byChild: "date").queryEnding(atValue: timestamp-1).queryLimited(toLast: 5)
        ref.observeSingleEvent(of: .value) { (snapshot) in
            guard let allObjects = snapshot.children.allObjects as? [DataSnapshot] else{
                return
            }
            // remove the indicator view if empty
